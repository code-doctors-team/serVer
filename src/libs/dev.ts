...
const watchFiles = (sv: any): void => {
  const [watchedPages, watchedCss, watchedJs] = [
    sv.watch(`./**/*${config.extname}`, {
  ...
  watchedCss.on("change", (path: string): void => {
    sv.reload(path, cssInjector.name, {
      url: urlSv,
      cssRoot: config.cssRoot,
    });
  });
  watchedJs.on("all", (): void => {
    sv.reload();
  });
};
...
export default async function dev({
  ...
  port,
  onlyReload,
}: OptionsDev): Promise<Promise<void>> {
    ...
    ({ url, snippet, urlExternal }): void => {
      ...
      if (browser) {
        ...
        if (typeof browser === "string") {
          ...
          if (browsersName.includes(formatedBrowser))
            browserSelected = browsers[formatedBrowser];
          else {
        ...
        open(url, {
          ...
          .then(({ app }) => {
            const trieds: string[] = name
              .slice(0, 1)
              .filter((browser) => browser !== app.name);
            const [app_name]: [string, string] = Object.entries(browsers).find(
              ([_, value]) => value === app.name
            );
            logger.logString`Opening browser {hex("#4BFF65") ${app_name}}${
              trieds.length
                ? logger.chalk` {hex("#FFFB26") (${trieds.join(
                    "|"
                  )}) Tried Browser}`
                : ""
            }${
              notAvaible
                ? logger.chalk` {hex("#FFFB26") (\`${notAvaible}\` not available browser)}`
                : ""
            }`;
          })
          .catch((err) => {
