...
import ejs from "ejs";
import { createReadStream, createWriteStream, existsSync, mkdir, ReadStream, WriteStream } from "fs";
...
const filterMatch: (match: RegExp) => (line: string) => number = (match: RegExp) => (line: string): number => line.match(match)?.length;
...
const MapLines: (Lines: string[], match: RegExp) => LineProcessed[] = (Lines: string[], match: RegExp) => (
  Lines
    .filter(filterMatch(match))
    .map((LinesMatched: string): LineProcessed => [parse(LinesMatched), LinesMatched])
);
                                
function ProcessCSS(LinksAndStrings: LineProcessed[], content: string, cfg: any,  config: configType, cb: () => void) {
  LinksAndStrings
  .forEach(([element, linkString]: LineProcessed, index: number): void => {
    const links: HTMLElement[] = element.querySelectorAll('link[rel="stylesheet"]');
     if(!links.length) {
...
function ProcessJS(ScriptsAndStrings: LineProcessed[], change: (scriptStringAtChange: string, newScript: string) => void): Promise<void> {
  const formatUrl = (url: string): string => url[0] !== '/' ? '/'.concat(url) : url;
  const scriptsAndString_promise: Promise<void> = new Promise(resolve => {
    let index_lines = 0;
    ScriptsAndStrings
      .forEach(async ([element, scriptString]: LineProcessed): Promise<void> => {
...
export default function pagesOperation(config: configType): Promise<void> {
  ...
  return new Promise(res => {
    ...
    converLoadDataToPromise
    .then(() => {
      const WriteStreamContent = new Writable({
        write(chunk, _encoding, cb) {
          const [absolutePath, url, relativePath]: string[] = chunk.toString().split(',');
  
          const objectRelativePath: ParsedPath = path.parse(relativePath);
          const { dir, name } = objectRelativePath;
  
          objectRelativePath.dir = `${dir ? dir.concat('/') : ''}${!defaults[name] ? name : ''}`.trim();
          objectRelativePath.base = defaults[name] === '404' ? '404.html' : 'index.html';
  
          const urlComplet: string = path.resolve(rootPages, path.format(objectRelativePath));
          ...
          const TransformContent = new Transform({
            write(chunk, _encoding, cb) {
                ...
                const LinksAndStrings: LineProcessed[] = MapLines(Lines, /<link/g);
                const ScriptsAndStrings: LineProcessed[] = MapLines(Lines, /<script/g);
  
                const status: StatusLine = {
                  NoLinksNoScripts: Boolean(!LinksAndStrings.length && !ScriptsAndStrings.length),
                  LinksAndScripts: Boolean(LinksAndStrings.length && ScriptsAndStrings.length),
                  OnlyLinks: Boolean(LinksAndStrings.length && !ScriptsAndStrings.length),
                  OnlyScripts:  Boolean(!LinksAndStrings.length && ScriptsAndStrings.length),
                };
                
                const { NoLinksNoScripts, LinksAndScripts, OnlyLinks, OnlyScripts } = status;
      ...
      RouterStream.on('end', () => {
        config.spinner.info.add(chalk`Found {hex("#4BBAFF") (${routes.size})} pages in directory ${config["#pagesRoot"]}`)
