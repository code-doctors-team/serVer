...
function textsForm(text) {
  if(text) {
    const v_text_before = this.prefixText.message.concat(!this.prefixText.value.length ? '' : ' ').concat(`V ${text}`);
    return [v_text_before];
  }
  const v_text_before = this.prefixText.message.concat(!this.prefixText.value.length ? '' : ' ').concat(`V ${this.cache}`);
  const v_text_after = this.prefixText.message.concat(!this.prefixText.value.length ? '' : ' ').concat(`V ${this['~text']}`);
...
export default class Spinner {
  constructor({ text = '', color = '', custom = {}, spinner = 'arc', prefixText = {
    value: '',
    message: '',
  } }) {
    this['@text'] = text;
  ...
  info = new Set();
  #callback = null;
  spinners = () => Object.assign(this.custom, {
    arc: {
      interval: 100,
      frames: [
        chalk`{blue ◜}`,
  ...
  start = (text) => {
    ...
    process.stdout.write(`${this.prefixText.value} ~ ${Reflect.get(this, '@text')}`);
    ...
    this.timer = setInterval(() => {
      ...
      let text = (!this.prefixText.value.length ? '' : ' ').concat(`${now} ${this.text}`);
  ...
  setTextPromise = (newText) => new Promise(res => {
    this.cache = this['~text'];
    rdl.clearLine(process.stdout, 1, () => {
      const [newTextNormal, newTextParsed] = newText.split('|'); 
      Reflect.set(this, '@text', newTextNormal)
  ...
  stop = (messageStop = '') => {
    ...
    process.stdout.write(
      (!this.prefixText.value.length ? '' : ' ')
        ...
        .concat(
          this.errors.size ? ' '
            .concat(chalk`{red {bold {italic Found ${this.errors.size} ${this.errors.size === 1 ? 'error' : 'errors'}}}}`): ''
        )
        .trimEnd()
        .concat('\n')
    );
