...
const isEqualElement = (oldFiber, element, areTexts) => {
  if (areTexts) {
    const sameTextNode = Boolean(
      oldFiber && element && oldFiber.dom.rawText === element.rawText
    );
    return sameTextNode;
  }
  const [sameType, sameProps] =
    (oldFiber &&
      element &&
      !areTexts && [
        element.nodeType === oldFiber.dom.nodeType &&
          oldFiber.tag === element.rawTagName,
...
const createIndex = (prevSibling, oldFiber, { isAction, index }) =>
  prevSibling?.action === Placement
    ? prevSibling.index
    : isAction.placement || isAction.deletion
    ? index
    : oldFiber.index;
...
export default function reconcileChildren(fiberParent, elements) {
  ...
  if (existingChildren.size && !elements.length) {
    if (
      !(
        fiberParent.action === Update &&
        fiberParent.tag === fiberParent.alternate?.tag
      )
    ) {
      addDeletion({
        action: Deletion,
  ...
  const ParentNotUpdateOrUpdateNotChange =
    fiberParent.action !== Update ||
    fiberParent.tag === fiberParent.alternate?.tag;
  ...
  while (index < elements.length) {
    ...
    if (isEqualElementAndFiber) {
      newFiber = {
        ...
        props: {
          ...(areTexts
            ? {
                nodeValue: oldFiber.dom.rawText.replace(
                  new RegExp("\r", "g"),
                  ""
                ),
              }
      ...
      if (nextElement && nextElement.rawTagName === newFiber.tag) {
        ...
        while (nextElement) {
          const isDiff = isEqualElement(
            ...
            ? isDifferent(newFiber.alternate, nextElement)
            : true;
        ...
        if (elementsAreEqual.length) {
          ...
          let isFind = false;
          while (nextOldFiber) {
            const isDiff = isEqualElement(nextOldFiber, element, areTexts)
              ? isDifferent(nextOldFiber, element)
              : true;
            if (!isDiff && existingChildren.size > elements.length) {
              isFind = true;
              break;
          ...
          if (isFind) {
            ...
            existingChildrenAtMutation.set(
              newFiber.alternate,
              newFiber.alternate
            );
            continue;
    ...
    if (oldFiber && element && !isEqualElementAndFiber) {
      ...
      while (nextElement) {
        const isEqualNextElementAndFiber =
          isEqualElement(oldFiber, nextElement, areTexts) &&
          existingChildren.size < elements.length &&
          !isDifferent(oldFiber, nextElement);
      ...
      if (oldFiberExisting) {
        ...
        isChanged = true;
        isAction.placement = true;
      } else if (existingChildrenAtMutation.has(oldFiber)) {
        let oldFiberElement = oldFiber.sibling;
        let isFind = false;
        while (oldFiberElement) {
          const sameElement =
            isEqualElement(oldFiberElement, element, areTexts) &&
            existingChildren.size > elements.length &&
            !isDifferent(oldFiberElement, element);
          if (sameElement) {
            if (!newFiber) {
              isFind = true;
              break;
